          ...
          {status === 'ok' ? (
            <div className="py-8 text-center">
              <div className="w-12 h-12 rounded-full border border-volt/30 flex items-center justify-center mx-auto mb-6">
                <svg className="w-5 h-5 text-volt" fill="none" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </div>
            ...
            <>
              <div className="flex items-start justify-between mb-8">
                <div>
                  ...
                  <p className="text-sm mt-1" style={{color:'rgba(240,237,232,0.45)'}}>Una conversación de 30 minutos puede cambiarlo todo.</p>
                </div>
                <button onClick={onClose} className="text-white/30 hover:text-white transition-colors p-1" aria-label="Cerrar">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 20 20"><path d="M5 5l10 10M15 5l-10 10" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>
                </button>
              </div>
              <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <input className={cn('field', errors.name && 'border-red-500/40')} placeholder="Tu nombre" {...register('name')} />
                    {errors.name && <p className="text-xs mt-1" style={{color:'rgba(248,113,113,0.7)'}}>{errors.name.message}</p>}
                  </div>
                  <div>
                    <input className={cn('field', errors.email && 'border-red-500/40')} placeholder="Email" type="email" {...register('email')} />
                    {errors.email && <p className="text-xs mt-1" style={{color:'rgba(248,113,113,0.7)'}}>{errors.email.message}</p>}
                  </div>
                </div>
                <input className="field" placeholder="Empresa (opcional)" {...register('company')} />
                <div>
                  <textarea className={cn('field resize-none', errors.message && 'border-red-500/40')} rows={4} placeholder="¿Cuál es el mayor reto operativo de tu empresa ahora mismo?" {...register('message')} />
                  {errors.message && <p className="text-xs mt-1" style={{color:'rgba(248,113,113,0.7)'}}>{errors.message.message}</p>}
                </div>
                {status === 'err' && <p className="text-sm" style={{color:'rgba(248,113,113,0.7)'}}>Algo falló. Escríbenos a luis@idalva.es</p>}
                <button type="submit" disabled={status === 'loading'} className={cn('btn-primary w-full justify-center', status === 'loading' && 'opacity-50 cursor-not-allowed')}>
                  {status === 'loading' ? 'Enviando…' : 'Enviar mensaje'}
                </button>
                <p className="text-center text-xs" style={{color:'rgba(240,237,232,0.2)'}}>Sin spam · Respuesta en menos de 24h</p>
              </form>
            </>
          )}
        </div>
